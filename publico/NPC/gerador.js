...
classes.forEach(i=>{
  elementos[i] = {}
  elementos[i]['checkbox'] = query(`#Generate_NPC [type="checkbox"].${i}`)
  elementos[i]['button'] = query(`#Generate_NPC img.${i}`)
  elementos[i]['label'] = query(`#Generate_NPC label.${i}`)
  elementos[i]['span'] = query(`#Generate_NPC span.${i}`)
})
...
button_salvar_NPC.onclick = async ()=>{
  ...
  query(`.container_de_nomes button[data-nome="${NPC.nome}"]`).style.opacity = 1
  await new Promise(resolve => setTimeout(resolve, 500));
  button_salvar_NPC.style.bordercolor = ''
  button_salvar_NPC.style.backgroundColor = ''
  await new Promise(resolve => setTimeout(resolve, 2000));
  query(`.container_de_nomes button[data-nome="${NPC.nome}"]`).style.opacity = ''
}
