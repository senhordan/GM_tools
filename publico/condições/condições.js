...
socket.on('conditions', (objs_condições)=>{
  ...
  objs_condições.forEach(condição=>{
      ...
      container_de_condições_nomes.innerHTML += "<button data-condição=\""+condição_nome.toLowerCase()+"\" data-grupo=\""+grupo+"\" class=\"btn-rdn nome_não_selecionado\" onclick=\"mostrar_esconder_condição('"+condição_nome.toLowerCase()+"')\">"+condição_nome+"</button>"
...
input_pesquisar_condição.oninput = ()=>{
  filtrar_condições(input_pesquisar_condição.value)
}
